...
<p></p>
where U is semi-orthogonal, i.e U<sup>T</sup>U = I, and V is orthogonal, i.e V<sup>T</sup>V = VV<sup>T</sup> = I. Here U is a m x n matrix, S is a diagonal n x n matrix and V is also a n x n matrix. Assume in all following sections that all matrices are real.
...
<p></p>
Using some trigonometric formulas, we arrive at the following conditions θ should satisfy to ensure that a'<sub>p</sub><sup>T</sup>a'<sub>q</sub> = 0:
<p></p>
...
First an overview.
 ...
 1. Computation of **Pseudo-Inverse** A<sup>-</sup> of matrix A according to the formula A<sup>-</sup> = VS<sup>-</sup>U<sup>T</sup>, where Pseudo-Inverse S<sup>-</sup> of S is easily computed by inverting non-vanished diagonal elements of S. It can immediately be checked that A<sup>-</sup> is the Pseudo-Inverse of A by checking that it satisfies AA<sup>-</sup>A = A.
...
<p></p>
If we have an overdetermined system of linear equations, 
<p></p>
<center> Ax = b. </center>
<p></p>
Now if the linear equations are linearly independent, then we can only hope to get an approximate solution minimizing || Ac - b ||. To solve this <strong>least squares</strong> problem we will use Singular Value Decomposition. For such a system A is tall, so we can apply the one-sided Jacobi algorithm to obtain,
<p></p>
<center> A = USV<sup>T</sup>. </center>
<p></p>
We want to minimize || Ac - b ||. This can be rewritten using projection operators UU<sup>T</sup> and 1 - UU<sup>T</sup>, onto the column space of A and its orthogonal component. Using this:
<p></p>
...
<p></p>
Once again let A be a m x n matrix, assumed to be tall. We will now discuss how to obtain a <strong>lower rank approximation</strong> to A. Then we can use the one-sided Jacobi algorithm to obtain a Singular Value Decomposition of A as above:
<p></p>
